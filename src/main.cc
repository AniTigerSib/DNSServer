...
int main(int argc, char **argv) {
    ...
    try {
        logger = new Logger(server_config.base_filename, server_config.max_log_size);
        ...
        server = new DNSServer(server_config.port, io_context, server_config.base_dns_ip, *logger);
...
void parseServerConfiguration(ServerConfiguration& p_conf, const std::string& conf_filename) {
    try {
        ...
        throw ConfigurateException("Error parsing YAML configuration: " + std::string(e.what()));
    } catch (const std::exception& e) {
        throw ConfigurateException("Error loading configuration file: " + std::string(e.what()));
    }
}