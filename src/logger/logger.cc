...
void Logger::findNextFileNumber() {
    ...
    try {
        ...
        for (const auto& entry : fs::directory_iterator(parent_dir.empty() ? "." : parent_dir)) {
            ...
            if (current_filename.find(basename) == 0) {
                ...
                if (underscore_pos != std::string::npos) {
                    // Извлекаем часть между последним подчеркиванием и расширением
                    std::string num_str;
                    if (!extension.empty()) {
                        size_t ext_pos = current_filename.find(extension, underscore_pos);
...
void Logger::processQueue() {
    if (!openNewLogFile()) {
        ...
        error_promise_.set_exception(
            std::make_exception_ptr(
                LoggerException("Failed to open initial log file: " + getCurrentFileName())
            )
        );
        return;
    }
    ...
    try {
        ...
        while (true) {
            ...
            if (!message.empty()) {
                size_t message_full_size = message.length() + 1; // +1 for new string symbol
